...
throw new Error("error" in data ? data.error : "Failed to load vendors");
}
...
<Link
href="/"
className="inline-flex items-center gap-2 bg-blue-600 text-white text-sm px-4 py-2 rounded-lg hover:bg-blue-700 transition"
>
<Home size={16} />
Home
</Link>
...
<h1 className="text-3xl font-extrabold">Find Vendors</h1>
<p className="mt-2 text-slate-600">Search by company name or services.</p>
...
<button
onClick={onSearch}
className="mt-3 bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg font-medium transition"
>
Search
</button>
</div>
...
{loading && <p className="text-slate-500 mt-4">Loading vendors...</p>}
{error && <p className="text-red-600 mt-4">Error: {error}</p>}
{!loading && !error && vendors.length === 0 && (
<p className="text-slate-500 mt-4">No vendors found.</p>
)}
