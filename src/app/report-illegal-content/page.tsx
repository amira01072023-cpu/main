...
export default function ReportIllegalContentPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMsg("Something went wrong. Please try again or email info@uaebizconnect.com.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <main className="min-h-screen bg-[#f8fafc] p-6 text-slate-800">
      <div className="max-w-2xl mx-auto bg-white border rounded-xl p-6 mt-8">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/eu-compliance" className="text-sm text-blue-600 hover:underline">EU Compliance</Link>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <button disabled={submitting} className="bg-blue-600 text-white px-5 py-2 rounded-lg disabled:opacity-60">{submitting ? "Submitting..." : "Submit Report"}</button>
        </form>
